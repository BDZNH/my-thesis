...
\begin{table}[!htbp]
    ...
    %\begin{tabular}{lcrr} 
        \begin{tabular}{l l ccc}%p{4em}<{\centering} p{5em}<{\centering} p{3em}<{\centering}}
        \toprule %\hline 
...
以图 \ref{fig:DFA1} 中的 DFA 为例，按照附录代码 \ref{lst:DFASample} 所示，实例化一个类 DFA 的对象之后，通过执行最小化函数，观察他们的输出结果，列出转移函数，绘制自动机状态转移图。对比输入输出结果以得出结论。类 DFA 中需要测试的五个最小化算法如表 \ref{tab:listMinAl} 所示。%%（表 \ref{tab:listMinAl} 中，Final-useful FA 可以通过执行类 DFA 的成员函数 DFA::usefulf() 得到）
...
\begin{table}[!htbp]
    ...
    \renewcommand{\arraystretch}{1.2}%row space 
    %\begin{tabular}{lcrr} 
        \begin{tabular}{lc} %l p{4em}<{\centering} p{3em}<{\centering} p{3em}<{\centering}
        \toprule
...
\begin{lstlisting}[language=C++,label={lst:hopcroft-iter-edit},caption={ min-hop.cpp },firstnumber=68]
    L[q]--;
...
\begin{lstlisting}[language=C++,label={lst:hoperrorResult},caption={}]
    DFA
    Q = [0,2)
    S = { 0 }
    F = { 1 }
...
图 \ref{fig:DFAMin-3-1} 原本是一个最小的 DFA，由表 \ref{tab:sample_3} 可知，算法 “DFA::min\_Hopcroft()” 已经改变了图 \ref{fig:DFAMin-3-1} 的状态数，由 “3” 变到 “2”， 如图 \ref{fig:DFAMinHoop-3} 所示。算法 “DFA::min\_Hopcroft()” 输出了错误的结果。
...
% 以附录图 \ref{fig:hopcerror} 中的五个最小的 DFA 为数据，测试结果统计如表 \ref{tab:KeepMinResultofAll} 所示
...
%     \setlength{\tabcolsep}{6pt}% column separation
%     \renewcommand{\arraystretch}{1.2}%row space 
%     \begin{tabular}{l| ccccc} %p{4em}<{\centering} p{4em}<{\centering} p{4em}<{\centering} p{4em}<{\centering} p{4em}<{\centering} }  %l p{3em}<{\centering} p{3em}<{\centering} p{3em}<{\centering}
%         \toprule %\hline 
...
% 对于附录图 \ref{fig:hoperror--1} ，修改后 Hopcroft 算法输出如下代码 \ref{FCode1}，如图 \ref{fig:hoperror--1-result} 所示。
...
% 对于附录图 \ref{fig:hoperror--2} ，无论是否修改，Hopcroft均输出代码 \ref{FCode2}，如图 \ref{fig:hoperror--2-result} 所示。
...
% 对于附录图 \ref{fig:hoperror--4} , 修改后 Hopcroft 输出代码 \ref{FCode4}，如图 \ref{fig:hoperror--4-result} 所示。
...
%   表 \ref{tab:KeepMinResultofAll-hop} 为输入的 DFA 在 Hopcroft 算法中的对比数据
...
%以图 \ref{fig:hoperror--2} 中的 DFA 为例，
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Function{split}{$p,q,c,P$} \Comment{$p \in repr$;$c \in V$;$P:= [A]_E$（定义\ref{def:equiv-class}）}
            ...
            \EndFor
            \If {$part \ne [p] \land part \ne \emptyset$}
                \State $P$.split($part$) \Comment{split 见 \ref{sec:test:stateeqrel} 节 }
...
% 由第 \ref{sec:reason-hopcroft} 的内容，可知构造完全 DFA，使得对于满足关系 $|F| > |Q \setminus F|$ 的非完全 DFA 成为完全 DFA 后，存在 $ c \in V $， $ p \in F$ ，使得 $  T(p,c) \in (Q\setminus F) $ 即可。
...
%     \item 构造完全 DFA，使得对于满足关系 $|F| > |Q \setminus F|$ 的非完全 DFA中，存在 $ c \in V $， $ p \in F$ ，使得 $  T(p,c) \in (Q\setminus F) $；
% \end{enumerate}
...
% 并且 Bruce William Watson 也对 Hopcroft 算法（算法 \ref{al:4-8}）中语句 2 进行了详细的说明，Bruce William Watson 已经指出，由于一开始就将 $Q$ 分割成了 $P=[Q]_{E_{0}}=\{Q \backslash F, F\}$ ,所以只需要相对于 $(Q \backslash F, b)$ 或 $(F, b)$ （对所有的 $b \in V $）进行等价类分割即可\cite{Hopc71,watson1993taxonomyb}。所以方案 1 并不是一个比较好的方法。
...
需要留意的是，FIRE engine 中仅实现了用于去除陷阱状态的函数 DFA::usefulf() ，而没有实现用于去除开始不可达状态的函数，虽然不可达状态并不会影响 DFA 接受的 $\mathcal{L} $。但是不可达状态同样会影响算法的时间复杂度（不可达状态增加了 DFA 的大小）。
...
\begin{table}[!htbp]
    ...
    \setlength{\tabcolsep}{4pt}% column separation
    \renewcommand{\arraystretch}{1.2}%row space 
    \begin{tabular}{c p{7em}<{\centering} p{4em}<{\centering} l}  %l p{3em}<{\centering} p{3em}<{\centering} p{3em}<{\centering}
        \toprule %\hline 
                序号  &  数据 & 属性 & 备注 \\
        \midrule%\hline
