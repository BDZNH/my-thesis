...
本小节总结如下
\begin{itemize}
    \item FIRE engine 对于算法 “DFA::min\_Hopcroft()” 和算法 “DFA::min\_Brzozowski” 的实现仍然有不足之处；
...
以表 \ref{tab:MinData} 中第三个数据，也即图 \ref{fig:DFAMin-3-1} 中的 DFA 为例，此 DFA 在 DFA::min\_Hopcroft 算法中的迭代过程如表 \ref{tab:hopcroftAna} 所示，算法原型见图 \ref{fig:desHopcroft}：
...
下面对 Hopcroft 算法进行分析：
\begin{enumerate}[(1)]
    \item $Q$ 首先被分为两个等价类，$P=(Q\setminus F) \cup F = \{(0),(1,2)\}$ 。因 $|Q\setminus F|\le |F|$，所以 $L=F\times V$；
    \item 第一次迭代，取出 $repr$ 第一个元素 $0$，赋值给$p$，此时 $p=0,q=0,C=b$，并且从$L$ 内移除与字符 'b' 相关的转移关系，$Splittable(p,q,c)$不成功，不做操作；
    ...
    \item 第五次迭代，此时 $L$ 内已经没有转移关系，算法结束。
\end{enumerate}